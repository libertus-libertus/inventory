...
@section('content')
    <div class="mb-8">
        ...
        <p class="text-gray-600">Riwayat transaksi barang keluar ke pelanaggan.</p>
    </div>

    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-lg font-semibold text-gray-900">Daftar Barang Keluar</h3>
        ...
        @else
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200" id="stock-outs-table">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($stockOuts as $stockOut)
                            <tr>
                                <td class="px-4 py-2 text-sm text-gray-700">{{ $stockOut->reference_code }}</td>
                                <td class="px-4 py-2 text-sm text-gray-700">{{ \Carbon\Carbon::parse($stockOut->stock_out_date)->format('d M Y') }}</td>
                                <td class="px-4 py-2 text-sm text-gray-700">{{ $stockOut->product->name }}</td>
                                <td class="px-4 py-2 text-sm text-gray-700">{{ $stockOut->customer->name ?? 'Pelanggan tidak ditemukan' }}</td>
                                <td class="px-4 py-2 text-sm text-gray-700">{{ $stockOut->quantity }} {{ $stockOut->product->unit }}</td>
                                <td class="px-4 py-2 text-sm text-gray-700">{{ $stockOut->user->name }}</td>
                                <td class="px-4 py-2 text-sm text-gray-700">
                                    <a href="{{ route('stock_outs.show', $stockOut->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-3" title="Lihat Detail">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($products as $product)
                            <tr>
                                ...
                                <td class="px-4 py-2">
                                    <a href="{{ route('stock_outs.create', ['product_id' => $product->id]) }}"
