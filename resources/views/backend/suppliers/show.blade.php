...
@section('content')
    <div class="mb-8">
        ...
        <p class="text-gray-600">Informasi lengkap mengenai supplier.</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex space-x-2">
                ...
                <form id="delete-form-{{ $supplier->id }}" action="{{ route('suppliers.destroy', $supplier->id) }}" method="POST" class="inline-block">
                    @csrf
                    @method('DELETE')
                    <button type="button" onclick="confirmDeleteSupplier({{ $supplier->id }}, '{{ $supplier->name }}')" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 focus:outline-none focus:border-red-900 focus:ring ring-red-300 disabled:opacity-25 transition ease-in-out duration-150">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <p class="mt-1 text-lg font-semibold text-gray-900">{{ $supplier->name }}</p>
            </div>
            <div>
                <p class="text-sm font-medium text-gray-500">Telepon:</p>
                <p class="mt-1 text-base text-gray-800">{{ $supplier->phone ?: '-' }}</p>
            </div>
            <div>
                <p class="text-sm font-medium text-gray-500">Alamat:</p>
                <p class="mt-1 text-base text-gray-800">{{ $supplier->address ?: 'Tidak ada alamat.' }}</p>
            </div>
            <div>
        ...
        <div class="mt-8 pt-6 border-t border-gray-200 flex justify-end">
            <a href="{{ route('suppliers.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-200 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition ease-in-out duration-150">
